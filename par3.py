...
class allRead:
    ...
    def __init__(self, infilename, start_ptr=0, end_ptr=0, byteswap=False):
        ...
        self.recs_categories = {'65': ['data.Time', 'data.Roll', 'data.Pitch', 'data.Heave', 'data.Heading'],
                                '71': ['data.Time', 'data.SoundSpeed'],
                                '73': ['time', 'settings'],
                                '78': ['time', 'header.SoundSpeed', 'header.Ntx', 'header.Nrx', 'header.Nvalid',
                                       'header.SampleRate', 'tx.TiltAngle', 'tx.SignalLength', 'tx.Delay',
                                       'tx.Frequency', 'tx.WaveformID', 'rx.BeamPointingAngle', 'rx.TransmitSectorID',
                                       'rx.DetectionInfo', 'rx.QualityFactor', 'rx.TravelTime'],
                                '80': ['time', 'header.Speed', 'header.Heading', 'header.Course'],
    ...
    def seek_next_startbyte(self):
        ...
        """
        # this easy check screwed me once, never again
        # easiest check that is valid most of the time, that you are at the start of a record.  Fifth byte should be '2'
        #cur_ptr = self.infile.tell()
        #self.infile.seek(4, 1)
        #stx = self.infile.read(1)
        #self.infile.seek(cur_ptr, 0)
        #if stx == b'\x02':
        #    self.at_right_byte = True

        # next check is to continue on until you find the header pattern, which surrounds the STX byte.  Can't just
    ...
    def sequential_read_records(self):
        recs_to_read = dict([(k, dict([(r.split('.')[-1].lower(), None) for r in self.recs_categories[k]])) for k in
                             self.recs_categories])

        self.infile.seek(self.start_ptr)
        while not self.eof:
            self.read()
            datagram_type = str(self.packet.dtype)
            # workflow for recs with packed data objects, time indexed
            if datagram_type in list(self.recs_categories.keys()):
                self.get()
                rec = self.packet.subpack
                for subrec in self.recs_categories[datagram_type]:
                    #  override for nested recs, designated with periods in the recs_to_read dict
                    if subrec.find('.') > 0:
                        tmprec = rec.__getattribute__(subrec.split('.')[0])
                        subrec = subrec.split('.')[1]
                    else:
                        tmprec = rec

                    try:  # flow for array/list attribute
                        val = list(tmprec.__getattribute__(subrec))
                    except TypeError:  # flow for float/int attribute
                        val = [tmprec.__getattribute__(subrec)]
                    ...
                    if recs_to_read[datagram_type][subrec.lower()] is None:
                        ...
                        recs_to_read[datagram_type][subrec.lower()].append(val[0])
        return recs_to_read
...
class BatchRead:
    def __init__(self, infilelist):
        self.flist = self._build_file_list(infilelist)
        self.recs = None

    def _build_file_list(self, infilelist):
        flist = []
        if type(infilelist) is str:
            infilelist = [infilelist]

        for fil in infilelist:
            if os.path.isfile(fil) and os.path.splitext(fil)[1] == '.all':
                flist.append(fil)
            elif os.path.split(fil)[1] == '*.all':
                flist.extend(glob(fil))
            else:
                print('Input {}: Not a Kongsberg file format'.format(fil))

        if not flist:
            raise EnvironmentError(r'File list provided does not contain .all files: {}'.format(infilelist))
        else:
            return flist

    def build_proc_records(self):
        for f in self.flist:
            allRead(f).sequential_read_records()
